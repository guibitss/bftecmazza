-- analysis_conversas ganha inbox_id (pro link abrir a conversa) e filtro por loja
DROP FUNCTION IF EXISTS analysis_conversas(text, timestamptz, timestamptz, int);

CREATE OR REPLACE FUNCTION analysis_conversas(
  p_tipo text, p_from timestamptz, p_to timestamptz,
  p_vendor int DEFAULT NULL, p_store int DEFAULT NULL
)
RETURNS TABLE(
