-- Lista as conversas por trás de uma métrica (para o "ver mais" das Métricas).
-- p_tipo:
--   sem_fechamento   → não fez pergunta de fechamento (poderia ter feito)
--   followup_perdido → cliente sinalizou "depois" e não houve retomada
--   negativa_seca    → falta de estoque respondida sem alternativa
--   esfriou          → lead esfriou/perdido
--   objecao_<tipo>   → objeção daquele tipo que não foi contornada
CREATE OR REPLACE FUNCTION analysis_conversas(
  p_tipo text, p_from timestamptz, p_to timestamptz, p_vendor int DEFAULT NULL
)
RETURNS TABLE(
  conversation_id int, vendor_id int, vendor_name text,
  customer_name text, last_message_at timestamptz,
  nota_geral int, desfecho text, preview text, sugestao text
) LANGUAGE sql STABLE AS $$
  SELECT
    ca.conversation_id, ca.vendor_id, v.name,
